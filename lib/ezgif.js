...
async function webp2mp4(source) {
    ...
    try {
        return await convert({
            type: 'webp-mp4',
            ...(isUrl ? { url: source } : { file: source, filename: randomName + ".webp" })
        });
    } catch (error) {
        console.error("Ezgif: webp-mp4 failed, trying webp-avif fallback.");
        try {
            return await convert({
                type: 'webp-avif',
                ...(isUrl ? { url: source } : { file: source, filename: randomName + ".webp" })
            ...
            console.error("Ezgif: webp-avif failed, trying webp-gif fallback.");
            try {
                return await convert({
                    type: 'webp-gif',
                    ...(isUrl ? { url: source } : { file: source, filename: randomName + ".webp" })
...
async function webp2img(source) {
    const urlRegex = /^(https?|ftp):\/\/[^\s/$.?#].[^\s]*$/;
    const isUrl = typeof source === 'string' && urlRegex.test(source);
    const randomName = crypto.randomBytes(5).toString("hex");
    ...
    try {
        ...
        console.error("Ezgif: webp-png failed, trying webp-jpg fallback.");
        try {
            return await convert({
                type: 'webp-jpg',
                ...(isUrl ? { url: source } : { file: source, filename: randomName + ".webp" })
...
module.exports = {
    ...
    webp2mp4,
    webp2img,
    webp2png: webp2img, // Alias for compatibility
    img2webp,
    vid2webp
};
