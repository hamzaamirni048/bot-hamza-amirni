...
function smsg(conn, m, store) {
    ...
    if (m.message) {
        ...
        // 🚨 FALLBACK: If it's a button response but still no prefix, and it's a known label, force it (Log for diagnostics)
        if (!m.body.startsWith('.') && (m.mtype.includes('Response') || m.mtype.includes('Reply'))) {
             console.log(`[smsg] ⚠️ Button ID missing prefix. Type: ${m.mtype} | Label/ID: "${m.body}"`);
             // Check if it's one of our specific hardcoded labels for "Developer"
             if (m.body.includes('Developer') || m.body.includes('المطور')) {
                  m.body = '.owner';
             }
        }
