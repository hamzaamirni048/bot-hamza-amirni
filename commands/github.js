...
async function githubCommand(sock, chatId, message) {
  try {
    // Fetch repo data
    const res = await fetch('https://api.github.com/repos/HamzabAmirni1/bot-hamza-amirni');
    if (!res.ok) throw new Error('Error fetching repository data');
    const repo = await res.json();

    // Format caption in styled output
    let txt = `*🌹 Hamza Amirni Repository*\n\n`;
    ...
    // Local bot image
    const imgPath = path.join(__dirname, '../media/hamza.jpg');
